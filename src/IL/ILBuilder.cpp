...
namespace Corrosive {
	...
	errvoid ILBuilder::build_const_dword(ILBlock* block, dword_t value) { block->write_instruction(ILInstruction::dword); 	  block->write_value(value.p1); block->write_value(value.p2); return err::ok; }
	...
	errvoid ILBuilder::build_roffset(ILBlock* block, ILDataType from, ILDataType to, ILSize offset) {
		if (offset.value > 0) {
			...
			if (!ILBuilder::build_bitcast(block, from, to)) return err::fail;
		}
		return err::ok;
